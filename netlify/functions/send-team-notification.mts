...
const handler = async (req: Request) => {
  try {
    ...
    if (!teamId) {
      return jsonResponse({ error: "teamId gerekli." }, 400);
    }
    ...
    const { data: targets, error: targetsError } = await supabase.rpc("get_push_targets_for_team", {
      p_team_id: teamId,
    });
    if (targetsError) {
    ...
    const title = body.title || "Rohan FC";
    const notificationBody = body.body || "Yeni bir bildirim var.";
    const payload = JSON.stringify({ title, body: notificationBody, url: body.url || "/" });
...
export const config: Config = {
  path: "/api/send-team-notification",
};
